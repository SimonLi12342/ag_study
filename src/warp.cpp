...
float tent_helper(float x) {
    return x < 0.5f ? sqrt(2.0f * x) - 1.0f : 1.0f - sqrt(2.0f - 2.0f * x);
}
Point2f Warp::squareToTent(const Point2f& sample) {
    return Point2f(tent_helper(sample.x()), tent_helper(sample.y()));
}

float tentPdf_helper(float t) {
    return t >= -1 && t <= 1 ? 1 - abs(t) : 0;
}
float Warp::squareToTentPdf(const Point2f& p) {
    return tentPdf_helper(p.x()) * tentPdf_helper(p.y());
}
...
float Warp::squareToBeckmannPdf(const Vector3f& m, float alpha)
{
    ...
    float temp_tan = (m.x() * m.x() + m.y() * m.y()) / (m.z() * m.z());
    float cosTheta_3 = m.z() * m.z() * m.z();
    float longitudinal = exp(-temp_tan / alpha_2) / (alpha_2 * cosTheta_3);
